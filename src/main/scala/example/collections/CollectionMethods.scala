...
applying the function you supply to each element, one at a time; 
...
creates a new list containing the elements that satisfy the provided predicate
...
comes from Lisp and other earlier functional programming languages. 
It’s used to access the first element (the head element) of a list.

head is a great method to work with, but as a word of caution it can
throw an exception when called on an empty collection.
...
def collectionMethodsheadExample() = 
    ...
    try
        emptyList.head            // throw an exception when called on an empty collection        
    catch
...
comes from Lisp and other earlier functional programming languages. 
used to get every element in a list after the head element.

tail throws a java.lang.UnsupportedOperationException if the list is empty.

no option like headOption for tail.

To safely work with the tail of a collection, 
you can use pattern matching or methods such as drop(1)
----------------------------------------------------------------------------
***************************************************************************/
...
- a nice way of “taking” the elements from a list that you want for new list.
...
- Best way to explain reduce is to create a little helper method you can pass
  into it
- An important concept to know about reduce is that its used to 
...
As a final note, if you’re using Java code in a Scala project, 
you can convert Java collections to Scala collections. By doing this you can
use those collections in `for` expressions, and can also take advantage of
