...
def futureAwaitExample(): Unit =
  ...
  println(
    s"Await.result returned: $n. Await.result throws exception if not finished in time"
  )
...
def futureAwaitReadyExample(): Unit =
  ...
  /* `ready` returns the same Future; when completed, `.value` is Some(Success/Failure) */
    ...
    _.fold(
      ...
      v =>
        println(
          s"Await.ready: $v. Doesn't throw exception. Returns Futures (Success/Failure)"
        )
    )
  )
