...
/** *********************************************************** What is
  ...
  *
  * Essence of Scala is a fusion of functional and object-oriented programming
  * in a typed setting:
  *
  ...
  * programming paradigm in which function definitions are trees of expressions
  * that each return a value, rather than a sequence of imperative statements
  * which change the state of the program.
  *
  * Functions are treated as first-class citizens, just like any other data
  * type:
  ...
  *
  * It can also be helpful to know that experienced functional programmers have
  * a strong desire to see their code as math, that combining pure functions
  * together is like combining a series of algebraic equations.
  *
  * Once you understand the paradigm, you want to write pure functions that
  * always return values, not exceptions or null values, so you can combine them
  * together to create solutions. That feeling that you're writing math-like
  * equations (expressions) is the driving desire that leads you to use only
  * pure functions and immutable values, because thats what you use in algebra
  * and other forms of math
  *
  * Immutable Values****************************
  *
  * In pure functional programming, only immutable values are used. In scala
  * this means
  *   - All variables are created as `val` fields
  *   - Only immutable collection classes are used, such as `List`, `Vector` and
  *     immutable `Map` & `Set` classes
  *
  * Using only immutable variables, how does anything ever change?
  *
  * When it comes to using collections, you apply a function to an existing
  * collection to create a new collection (like map & filter)
  */
...
/** ***********************************************************
  * ****************Pure Functions*******************************
  * ************************************************************* A pure
  * function is a function that depends only on its declared inputs and its
  * implementation to produce its output. It only computes its output and does
  * not depend on or modify the outside world.
  *
  ...
  *
  * Example, you can call a double function an infinite number of times with the
  * input value 2, and you’ll always get the result 4.
  *
  * Given that definition, as you can imagine, methods like these in the
  * scala.math._ package are pure functions:
  *   - abs
  *   - ceil
  *   - max
  *
  ...
  *   - map and many more.
  *
  * In Scala, functions and methods are almost completely interchangeable, so
  * even though we use the common industry term “pure function,” this term can
  * be used to describe both functions and methods. If you’re interested in how
  * methods can be used like functions, see the `Eta Expansion` discussion.
  *
  * Examples of Impure Functions
  *   - `println` -> methods that interact with console, files, db, web
  *     services, senors, etc... are all impure
  *   - `currentTimeMillis` -> date and time related methods are all impure b/c
  *     their output depends on something other than their input parameters
  *   - `sys.error` -> exception throwing methods are impure b/c they don't
  *     simply return a result
  *
  * Impure functions often do one or more of these things:
  *
  *   1. Read from hidden state i.e., they access variables and data not
  *      explicitly passed into the function as input parameters
  *   2. Write to hidden state
  *   3. Mutate the parameters they’re given, or mutate hidden variables, such
  *      as fields in their containing class
  ...
  * Because those functions do not return anything, logically the only reason
  * you ever call it is to achieve some side effect. In consequence, often the
  * usage of those functions is impure.
  *
  * Impure functions are needed. Write the core of your application using pure
  * functions, and then write an impure “wrapper” around that core to interact
  * with the outside world. As someone once said, this is like putting a layer
  * of impure icing on top of a pure cake.
  *
  * It’s important to note that there are ways to make impure interactions with
  * the outside world feel more pure. For instance, you’ll hear about using an
  * `IO` Monad to deal with input and output. These topics are b beyond the
  * scope of this, so to keep things simple it can help to think that FP
  * applications have a core of pure functions that are wrapped with other
  * functions to interact with the outside world.
  */
...
/** ***********************************************************
  ...
  * *************************************************************
  *
  * While every programming language ever created probably lets you write pure
  * functions, a second important Scala FP feature is that you can create
  * functions as values, just like you create String and Int values
  *
  * Benefits
  *   1. You can define methods to accept function parameters
  *   2. You can pass functions as parameters into methods
  *
  * Technically, a function that takes another function as an input parameter is
  * known as a Higher-Order Function..
  *
  * The ability to pass functions into other functions helps you create code
  * that is concise and still readable — expressive.
  */
...
/** ***********************************************************
  ...
  * *************************************************************
  *
  * Functional programming is like writing a series of algebraic equations, and
  * because algebra doesn’t have null values or throw exceptions, you don’t use
  * these features in FP.
  *
  * In the situations where you might normally use a null value or exception in
