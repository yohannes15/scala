...
/** ********************************************************************
  ...
  *     trade the advantages of static typing for simpler schemes where column
  *     names are represented as strings and passed to other operators, e.g.
  *     row.select("columnName"). This approach forgoes the advantages of static
  *     typing, and is still not as natural as the dynamically typed version.
  *     *********************************************************************
  *     ********************************************************************
  */
...
def structuralTypeExample() =
  ...
  println(
    s"${emma.selectDynamic("name")} is ${emma.selectDynamic("age")} years old."
  )
...
def structuralTypeExample2() =
  ...
  // Step 2: access fields the untyped way
  println(
    s"${bookRecord.selectDynamic("title")} has rating of ${bookRecord.selectDynamic("rating")}"
  )
...
/** ***********************************************
  *
  * Summary ------------------ Scala has several other advanced types that are
  * not shown in this book, including:
  *
  * Type lambdas Match types Existential types Higher-kinded types Singleton
  * types Refinement types Kind polymorphism
  *
  * For more details on most of these types, refer to the Scala 3 Reference
  * documentation. (https://docs.scala-lang.org/scala3/reference/overview.html)
  *
  * For singleton types see the literal types
  * (https://scala-lang.org/files/archive/spec/3.4/03-types.html#literal-types)
  * section of the Scala 3 spec, and for refinement types
  * (https://scala-lang.org/files/archive/spec/3.4/03-types.html), see the
  * refined types section.
  */
