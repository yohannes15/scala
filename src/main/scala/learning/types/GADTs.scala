...
/** *****************************************************************
  ...
  * ------------------------------------------------------------------- A
  * regular ADT (like OptionAdt[T]) has a type parameter T that is left open —
  * the caller decides what T is, and all cases share that same T.
  *
  * A GADT goes further: each case can FIX T to a specific concrete type via an
  * explicit `extends` clause. The compiler tracks which T belongs to which case
  * and uses that knowledge inside pattern match branches.
  *
  ...
  * enum Pair[A, B]: case Both(a: A, b: B) // A and B remain whatever the caller
  * passes in
  *
  * // GADT — each case pins T independently enum Expr[T]: case Num(n: Int)
  * extends Expr[Int] case Bool(b: Boolean) extends Expr[Boolean] case Add(l:
  * Expr[Int], r: Expr[Int]) extends Expr[Int] case IfThenElse(cond:
  * Expr[Boolean], thenB: Expr[T], elseB: Expr[T]) extends Expr[T]
  *
  * Plain ADT — T stays the same throughout. GADT — each case can narrow T to
  * something more specific.
  *
  * Because T is pinned per-case, the compiler can type-check functions over
  * GADTs WITHOUT any runtime casts. Each match arm refines T and the return
  * type is verified statically.
  */
...
def extract[T](b: Box[T]): T =
  ...
  b match
    ...
    case StringBox(s) =>
      StringBuilder(s"Happy birthday, $s").toString // T = String
