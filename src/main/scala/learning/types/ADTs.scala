...
/** ********************************************************************
  * **********************************************************************
  * Algebraic Data Types (ADTs) ------------------------------------ The enum
  * concept is general enough to also support ADTs and their generalized
  * versions (GADTs). Below is an example that shows how an `Option`-like type
  * can be represented as an ADT (`OptionAdt`).
  *
  * Example creates:
  *   - an `OptionAdt` enum (named to avoid clashing with `scala.Option` in this
  *     package) with a covariant type parameter `T` consisting of two cases,
  *     `Some` and `None`
  *   - `Some` is parameterized with a value parameter x: T. This is shorthand
  *     for writing a `case` class that extends OptionAdt
...
enum MyList[+A]:
  ...
  case Cons(
      head: A,
      tail: MyList[A]
  ) // e.g. Cons(1, Cons(2, Cons(3, Nil))) == [1,2,3]
