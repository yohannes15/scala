...
/** *****************************************************************
  ...
  * ------------------------------------------------------------------- Every
  * `enum` is syntactic sugar. The compiler expands it into:
  *
  *   1. A `sealed abstract class` extending `scala.reflect.Enum` (which
  *      requires each case to implement `def ordinal: Int`)
  *   2. A companion `object` containing the expanded cases
  *
  * Enum cases fall into three categories (see
  * https://docs.scala-lang.org/scala3/reference/enums/desugarEnums.html):
  *
  * SIMPLE CASE — name only, no params, no extends clause e.g. case North
  * desugars to: val North = $new(n, "North") ($new is a private factory that
  * creates the singleton)
  *
  * VALUE CASE — has an explicit `extends` clause (and/or body) but no parameter
  * section e.g. case Red extends ADTColor(0xFF0000) desugars to: val Red = new
  * ADTColor(...) { def ordinal = n }
  *
  * CLASS CASE — has a parameter section (may also have extends) e.g. case
  * Mix(mix: Int) extends ADTColor(mix) desugars to: final case class Mix(...)
  * extends ADTColor(mix) { def ordinal = n }
  *
  * Singleton cases (simple + value) are expanded to `val` definitions. Class
  * cases are expanded to `final case class` definitions.
  *
  * Note: the Scala 3 book
  * (https://docs.scala-lang.org/scala3/book/types-adts-gadts.html) shows a
  * simplified desugaring using `case object` for readability. The actual
  * compiler output uses anonymous `val`s for singleton cases. Both represent
  * the same concept — a named singleton with a fixed ordinal.
  *
  * Using `enum` is preferred over manual encoding because the compiler also
  * generates `values`, `valueOf`, and `fromOrdinal` utilities for free.
  */
...
object ADTColor2:
  ...
  final case class Mix(mix: Int) extends ADTColor2(mix) {
    override def ordinal = 3
  }
