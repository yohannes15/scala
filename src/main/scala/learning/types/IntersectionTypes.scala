...
/** **********************************************
  *
  * Intersection Types (Scala 3 only) ------------------------------------ Used
  * on types, the `&` operator creates a so called intersection type. The type
  * `A & B` represents values that are both of the type `A` and of the type `B`
  * at the same time.
  *
  ...
  * the members of B. Therefore, as shown, Resettable & Growable[String] has
  * member methods `reset` and `add`.
  *
  * Intersection types can be useful to describe requirements structurally. That
  * is, in our example f, we directly express that we are happy with any value
  * for x as long as it's a subtype of both Resettable and Growable. We did not
  * have to create a nominal helper trait Both[A] like the following:
  */
...
/** Mixes the same two capabilities without extending `Both` — structural match,
  * nominal mismatch.
  */
...
def intersectionTypesExample(): Unit =
  ...
  println(
    "goodf accepts any Resettable & Growable[String]; badf only Both[String]"
  )
