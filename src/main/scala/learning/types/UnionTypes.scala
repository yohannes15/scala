...
/** **********************************************************
  ...
  *
  * The | operator creates a so-called union type. The type A | B represents
  * values that are either of the type A or of the type B.
  *
  * In the following example, the help method accepts a parameter named id of
  * the union type `Username | Email`, that can be either a Username or an
  * Email.
  *
  * As shown, union types can be used to represent alternatives of several
  * different types, without requiring those types to be part of a
  * custom-crafted class hierarchy, or requiring explicit wrapping.
  *
