...
def listCollection(): Unit =
  ...
  // mixed types
  val things: List[String | Int | Double] =
    List(1, "two", 3.0) // with union types
  ...
  sampleList.dropWhile(
    _ < 25
  ) // List(30, 40, 10)     // drops while condition true
  sampleList.filter(
    _ < 25
  ) // List(10, 20, 10)     // keeps elements matching predicate
  sampleList.slice(
    2,
    4
  ) // List(30, 40)         // elements in index range [2,4)
  sampleList.tail // List(20, 30, 40, 10) // all but the head
  sampleList.take(3) // List(10, 20, 30)     // first 3 elements
  sampleList.takeWhile(
    _ < 30
  ) // List(10, 20)         // take while predicate true
