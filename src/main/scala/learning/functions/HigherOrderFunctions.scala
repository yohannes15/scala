...
/** **********************************************************
  ...
  * Functions (HOFs)
  * -------------------------------------------------------------------- A
  * higher-order function (HOF) is often defined as a function that: (a) takes
  * other functions as input parameters, or (b) returns a function as a result.
  *
  * In Scala, HOFs are possible because functions are first-class values. This
  * applies to both methods and functions due to Scala's eta-expansion.
  *
  * -------------------------------------------------------------------
  * Understanding `filter`'s Scaladoc
  * ------------------------------------------------------------------- Here's
  * the filter definition in the List[A] class:
  *
  * `def filter(p: A => Boolean): List[A]` // uses the predicate p to create and
  * return the List[A]
  *
  ...
  *
  * `p: A => Boolean` -> the function must take a value of type A and return a
  * Boolean
  *
  * So if your list is a List[Int], you can replace the type parameter A with
  * Int, and read that signature like this: `p: Int => Boolean`
  *
  * Because isEven has this type — it transforms an input Int into a Boolean —
  * it can be used with filter.
  *
  * ------------------------------------------------------------------- Writing
  * methods that take function parameters
  * -------------------------------------------------------------------
  *   - f is the name of the function input parameter.
  *   - The type signature of f specifies the type of the functions this method
  *     will accept.
  *   - The () portion of f's signature (on the left side of the => symbol): f
  *     takes no input parameters.
  *   - The Unit portion of the signature (on the right side of the => symbol):
  *     f should not return a meaningful result.
  *   - In the body of sayHello (on the right side of the = symbol), the f()
  *     statement invokes the function that was passed in.
  *     ***********************************************************
  *     **********************************************************
  */
