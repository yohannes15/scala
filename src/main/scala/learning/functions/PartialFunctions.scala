...
- Implements the `PartialFunction[A, B]` trait where A is the input
  type and B is the result type.
- Uses `case` expressions instead of a regular function body.

Key methods:
  isDefinedAt(x)          — returns true if the function handles x
  applyOrElse(x, default) — applies the function or falls back to the default
  orElse(other)           — chains two partial functions; tries `other`
                            for inputs not handled by the first
************************************************************
************************************************************/
...
def partialFunctionExample() =
    ...
    println(doubledOdds.applyOrElse(4, _ + 1))  // 5 (4 + 1, since 4 is not in domain)
