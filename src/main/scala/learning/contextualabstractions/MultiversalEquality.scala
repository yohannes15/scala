...
case class PrintedBook(author: String, title: String, year: Int, pages: Int)
    extends Book:
...
given CanEqual[PrintedBook, PrintedBook] =
  CanEqual.derived // allow `PrintedBook == PrintedBook`
given CanEqual[AudioBook, AudioBook] =
  CanEqual.derived // allow `AudioBook == AudioBook`
...
def realWorldExample() =
  ...
  println(
    s"$aBook and $pBook are equal: ${aBook == pBook}"
  ) // true (works because of `equals` in `AudioBook`)
  println(
    s"$pBook and $aBook are equal: ${pBook == aBook}"
  ) // true (works because of `equals` in `PrintedBook`)
