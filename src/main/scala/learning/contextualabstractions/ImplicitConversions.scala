...
/** **********************************************************************
  ...
  *   - Powerful scala feature that allows user to supply an argument of one
  *     type as if it were another type, to avoid boilerplate.
  *   - Note that in Scala 2, implicit conversions were also used to provide
  *     additional members to closed classes
  *
  ...
  * methods instead of implicit conversions (although the standard library still
  * relies on implicit conversions for historical reasons).
  *
  * Consider a method `findUserById` that takes a param of type Long. In scala,
  * it is possible to call the method with an arg of type `Int` instead of the
  * expected type Long, because the argument will be implicity converted into
  * the type Long.
  *
...
/** ************************************************************* Implicit
  * conversions are applied in two situations:
  * -------------------------------------------------------
  *   1. If an expression e is of type S, and S does not conform to the
  *      expression’s expected type T. a conversion `c` is searched for, which
  *      is applicable to e and whose result type conforms to T. In our example
  *      above, when we pass the argument id of type Int to the method
  *      findUserById, the implicit conversion int2long(id) is inserted.
  *   2. In a selection `e.m` with `e` of type `S`, if the selector `m` does not
  *      denote a member of `S` (to support Scala-2-style extension methods). a
  *      conversion `c` is searched for, which is applicable to e and whose
  *      result contains a member named `m`.
  *
  * An example is to compare two strings "foo" < "bar". In this case, String has
  * no member <, so the implicit conversion below is inserted. `scala.Predef` is
  * automatically imported into all Scala programs. ->
  * `Predef.augmentString("foo") < "bar"`
  *
  * How Are Implicit Conversions Brought Into Scope?
  * ------------------------------------------------------- When the compiler
  * searches for applicable conversions:
  *
  ...
  * S or the expected type T. The companion objects associated with a type X
  * are:
  ...
  * scope
  *
  * import Conversions.fromStringToUser import Conversions._
  *
  * In the introductory example, the conversion from Int to Long does not
  * require an import because it is defined in the object Int, which is the
  * companion object of the type Int.
  *
