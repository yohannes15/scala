...
/** ***************************************************************** Scala
  * offers two important features for contextual abstraction:
  *   - Context Parameters:
  *   - Given Instances (Scala 3) / Implicit Definitions (Scala 2): Terms that
  *     can be used by the Scala compiler to fill in the missing arguments
  *
  * Context Parameters ------------------------------------------------ Allow
  * you to specify parameters that, at the call site, can be omitted by the
  * programmer and should be automatically inferred/provided by the
  * context/compiler
  *
  * When designing a system, often context info like `configuration` or
  * `settings` need to be provided to the different components of your system.
  * One common way to achieve this by passing the config as an additional arg/s
  * to your methods. This is tedious
  *
  * Let us assume that the configuration does not change throughout most of our
  * code base. Passing config to each and every method call (like renderWidget)
  * becomes very tedious and makes our program more difficult to read, since we
  * need to ignore the config argument.
  */
...
def tediousRenderWidget(items: List[String], config: Config): String =
  "tediousRenderWidget"
