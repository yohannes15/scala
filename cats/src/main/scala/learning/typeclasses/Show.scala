...
/** Show is an alternative to the Java `toString` method. It is defined by a
  * single function:
  *
  * def show[A](a: A): String = ???
  *
  * toString is defined on `Any`(Java's Object) and can therefore be called on
  * anything! This is unwanted behaviour most often, as the standard
  * implementation of `toString` on non case classes is mostly gibberish: eg
  * `toStringIsGiberish` function below.
  *
  * The fact that toStringIsGiberish code compiles is a design flaw of the Java
  * API. We want to make things like this impossible, by offering the `toString`
