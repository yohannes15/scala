...
/** ******************************************************************************
  * Type constructors
  * -----------------
  * Values vs types vs "things that take types"
  * - A value has a type: 42 has type `Int`
  * - *Proper types* are complete by themselves: `Int`, `String`, `Boolean`.
  * - A type constructor is not a full type until you give it another type,
  *  it has a "hole" to fill.
  *
  ...
  * type arguments to get a proper type. `Option` and `List` are type constructors:
  * `Option` by itself is not the type of any runtime value — you write `Option[Int]`,
  * `Option[String]`, etc.
  *
  ...
  *
  * `F[_]` in Cats means: "`F` is a unary type constructor" (one hole). Then `F[A]` and `F[B]`
  * are proper types.
  *
  * `F` vs `f` in `Functor` (do not conflate these):
  * - `F` names the *type constructor* (`Option`, `List`, …): it describes the shape of a type,
  *  not a runtime function from `A` to `B`. So `F` is *not* a value-level `A => B`.
  * - `Functor`'s `map` has the form `def map[A, B](fa: F[A])(f: A => B): F[B]`. Here `f: A => B`
  * is* an ordinary Scala function: you pass it to `map` to turn each inner `A` into a `B`
  *  while staying inside `F`. That is the function you *map* with.
  *
  *  In short: `F` types the wrapper; `f` is what you run on the contents when you map.
  *
  * Kinds (optional vocabulary): `Int` has kind `*`; `Option` has kind `* -> *`. Saying
  * "F has kind `* -> *`" is the same as "`F` is a unary type constructor."
  * ******************************************************************************
  */
