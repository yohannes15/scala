...
/**
  ...
  * signature uses Any (Java's Object) to compare two values. This means that
  * we can compare two completely unrelated types without getting a compiler error.
  * 
  ...
  * 
  * E.g "Hello" == 42 or 42 == "Hello"
  * // warning: comparing values of types Int and String using `==`
  ...
  * Implementing `Eq` instances yourself for every data type might seem like
  * a huge drawback compared to only slight gains of typesafety. Fortunately,
  * we have two great options:

    1. Use inbuilt helper functions
    2. Use library called `kittens`
       -> automatic type class derivation for Cats including `Eq`
  */ 
...
@main def eqExamples() = 
    ...
    println(s"== equality check for case class Foo: ${Foo(10, "") === Foo(10, "")}")
