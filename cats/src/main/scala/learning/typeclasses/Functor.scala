...
/** Functor ----------
  ...
  *     structure by hand.
  *
  * E.g: List, Option, Either[E, *], Future, etc are all functors b/c they have
  * a sensible map
  ...
  *     (missing value, list of possibilities, async, errors on the left, etc.).
  *     Functor is “apply a pure function through that context.”
  *   - Look at `TypeConstructor.scala` for more.
  *
  * A `Functor` instance must obey 2 laws: --------------------------------
  *   - Composition
  *
  * Given `fa: F[A]`, `f: A => B`, and `g: B => C`, you can transform the inner
  * value in two equivalent ways:
  *
  * (1) Two maps: apply `f` inside `F`, then `g` — `fa.map(f).map(g)` → `F[C]`.
  *
  ...
  * `map` must not smuggle in extra behavior between the two steps; practically,
  * implementations may fuse consecutive maps without changing meaning
  ...
  * `fa.map(x => x) == fa`
  *
  * Together with composition, these laws pin down what “mapping” is allowed to
  * mean for a lawful functor.
  *
  ...
  * respect to fundamental functions like `map`. For instance `Option`'s effect
  * abstracts away potentially missing values, where `map` applies the function
  * only in the `Some` case but otherwise threads the `None` through.
  *
  * Taking this view, we can view `Functor` as the ability to work with a single
  * effect — we can apply a pure function to a single effectful value without
  * needing to "leave" the effect. This matches the mental model above: `F` is
  * the context (optional value, list, async, …) and `map` applies an `A => B`
  * through that context without unpacking to a bare `A` first. The quotes on
  * "leave" mean you remain inside the same structure (e.g. still `Option`,
  * still `Future`); you are not duplicating `map`-by-hand logic, even though
  * you stay in the effect.
  */
trait Functor2[F[_]]:
...
given functorForOption: Functor2[Option] = new Functor2[Option]:
...
/** After `FunctorsComposeExample`: `Functor[List].compose[Option].map(...)` still uses the raw
  * `List[Option[A]]` value — no extra wrapper. That breaks down when code is generic in `F`
  * and expects a `Functor[F]`: when `F` is “list on the outside, option on the inside”, i.e.
  * `F[A]` is `List[Option[A]]`, Scala has no one standard name for that `F`, so you
  * either introduce an alias + `given Functor[...] = compose` (as in `foo` below) or hand the
  * instance in with `using` — the “pass the composed instance / local implicit” situation.
  *
  * The second half uses `cats.data.Nested` instead: a dedicated type so the functor instance
  * is found like any other, traded off against wrapping/unwrapping noise at the value level.
  */
