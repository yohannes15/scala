...
/** If a type A can form a Semigroup it has an associative binary operation.
  * Associativity means the following equality must hold for any choice of x, y,
  * and z. combine(x, combine(y, z)) = combine(combine(x, y), z). Cats provides
  * many Semigroup instances out of the box such as `Int (+)` and `String (++)`
  *
  * trait Semigroup[A] { def combine(x: A, y: A): A }
  *
  * Cats defines the `Semigroup` type class in cats-kernel. The cats package
  * object defines type aliases to the Semigroup from cats-kernel, so that you
  * can simply import cats.Semigroup.
  */
@main def semiGroupExample() =
  ...
  println(
    s"The type for Semigroup[(List[Foo], Int)] is ${Semigroup[(List[Foo], Int)]}"
  )
...
def mergeMapExamples() =
  ...
  val y: Map[Int, List[String]] =
    mergeMap(mm1, mm2) // Map(2 -> List("cats"), 1 -> List("hello", "world"))
  println(y)
