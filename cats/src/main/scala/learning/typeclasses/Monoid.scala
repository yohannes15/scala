...
object Pair:
  ...
  /** If `Monoid[A]` and `Monoid[B]` exist, build `Monoid[Pair[A,B]]` pointwise
    * (tuple monoid).
    */
  given monoidPair[A: Monoid, B: Monoid]: Monoid[Pair[A, B]] =
    new Monoid[Pair[A, B]]:
      ...
      def combine(x: Pair[A, B], y: Pair[A, B]): Pair[A, B] =
        Pair(
          Monoid[A].combine(x.first, y.first),
          Monoid[B].combine(x.second, y.second)
        )

/** Fold with the monoid identity as the `foldRight` seed; `A` names the
  * implicit `Monoid[A]` (same as `using m: Monoid[A]`).
  */
def combineAll[A: Monoid](list: List[A])(using A: Monoid[A]): A =
  list.foldRight(A.empty)(A.combine)

/** this can be further synatic sugarized to below, but see note about cost for
