...
/** [[https://typelevel.org/cats-effect/docs/concepts#concurrent]]). Run:
  * `sbt "cats/runMain learning.effect.concepts.LearningConcurrency"`.
  */
object LearningConcurrency extends IOApp.Simple:
  ...
  /** Two *independent* effect-shaped values composed sequentially: total delay
    * ≈ sum.
    */
  def independentEffectsSequentialThenParTupled(): IO[Unit] =
    ...
    IO.println(
      "1) Same two independent service IOs: sequential flatMap (≈300ms) then parTupled (≈150ms)."
    ) >>
      sequential.flatMap { case (x, y) =>
        IO.println(s"   sequential: ($x, $y)")
      } >>
  ...
  def startAndJoin(): IO[Unit] =
    val child =
      IO.println("   (child) running...") >> IO.sleep(250.millis) >> IO.pure(7)
    IO.println(
      "2) start + join: parent does other work, then waits for the child outcome."
    ) >>
      child.start.flatMap { fib =>
        IO.println("   (parent) doing a bit of work...") >> IO.sleep(
          100.millis
        ) >> fib.join.flatMap {
          ...
          case Outcome.Canceled() =>
            IO.println("   (parent) child was canceled\n")
        }
      }

  /** `Ref` is a mutable *cell* you only touch through `IO`: each `get`, `set`,
    * and `update` is its own effect, so several fibers can share one ref
    * without the usual ad-hoc locking story. Common pattern: one ref for a
    * piece of shared state for the lifetime of a scope. Cats Effect implements
    * `Queue`, `Semaphore`, and friends on top of `Ref`. It is a low-level
    * primitive— application code that threads many refs can become hard to
    * read—so prefer a higher-level tool when it matches your use case.
    */
  def refUpdate(): IO[Unit] =
    IO.ref(0).flatMap {
      r => // initial value 0; in larger programs one ref is often shared across fibers in a scope
        IO.println("3) Ref: shared counter updated on this fiber.") >>
          r.update(_ + 1) >> r.get.flatMap(n => IO.println(s"   get: $n\n"))
    }

  /** `Deferred` is a handshake between fibers: one side calls `get` and its
    * `IO` pauses there until another side calls `complete` with a value—like
    * leaving a note “call me when you’re done” instead of asking over and over.
    * Only the first successful `complete` wins; after that, this `Deferred` is
    * done (create a new one for another round).
    *
    * `get` suspends the fiber without busy-waiting (it obtains the value of the
    * Deferred, or waits until it has been completed. The returned value may be
    * canceled.)
    *
    * `tryGet` checks once and moves on, often `None` until someone completes
    * it. Like `Ref`, it is a small generic building block; use clearer domain
    * types when they communicate intent better.
    */
  def deferredHandoff(): IO[Unit] =
    IO.deferred[String].flatMap { gate =>
      ...
      IO.println(
        "4) Deferred: main waits (fiber suspended on get) until the worker completes the gate."
      ) >>
