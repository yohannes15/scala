...
/** To create a new Cats Effect app ----------------------------------
  ...
  * multithreading, and all of the bells and whistles that you would expect from
  * a full application.
  */
object HelloWorld extends IOApp.Simple:
  val run = IO.println("Hello, World")

/** Here's a very silly version of FizzBuzz which runs four concurrent
  * lightweight threads, or fibers, one of which counts up an Int value once per
  * second, while the others poll that value for changes and print in response.
  *
  * We will learn more about constructs like `start` and `*>` in later pages,
  * but for now notice how easy it is to compose together concurrent programs
  * based on simple building blocks. Additionally, note the reuse of `wait` and
  * `poll` program.
  *
  * Because we're describing our program as a value (an `IO`), we can reuse that
  * value as part of many different programs, and it will continue to behave the
  * same regardless of this duplication.
  */
...
/** Use the REPL to understand IO / effects a little easier in a simpler
  * context. The unsafeRunSync() function is not meant to be used within a
  * normal application. As the name suggests, its implementation is unsafe in
  * several ways, but it is very useful for REPL-based experimentation and
...
def scalaReplExampleToCopyAndPlayWith() =
  ...
  lazy val loop: IO[Unit] =
    IO.println("loop until cancel..") >> IO.sleep(4.seconds) >> loop
  val cancel =
    loop.unsafeRunCancelable() // type cancel() in the REPL to stop it
  /*
  unsafeRunCancelable()
  -----------------------------
...
/** ***************************** TESTING ************************************
  ...
  * Test
  *
  * With this dependency, you can now write unit tests which directly return IO
  * programs without being forced to run them using one of the unsafe functions.
  * tests run more efficiently (since MUnit can run them in parallel).
  *
  * MUnit + cats-effect tests live under `cats/src/test/scala/` (e.g.
  * [[ExampleSuite]]).
  */
