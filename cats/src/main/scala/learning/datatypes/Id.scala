...
/** The identity monad can be seen as the monad that encodes the effect of
  * having no effect. plain pure values are values of Id. It is encoded as:
  *
  * `type Id[A] = A`
  *
  * That is to say that the type `Id[A]` is just a synonym for `A`. We can
  * freely treat values of type `A` as values of type `Id[A]` and vice versa.
  *
  * Using this type declaration, we can treat our `Id` type constructor as a
  * Monad and as a Comonad. The `pure` method, which has type `A => Id[A]` just
  * becomes the identity function. The map method from Functor just becomes
  * function application:
  *
  * For Monad, `flatMap` on `Id` is also just ordinary function application:
  * there is no nested structure to flatten. That is why `Id` is a good mental
  * model: the monad laws hold, but every step is "do nothing extra".
  */
object LearningId:
  ...
  def mapAndFlatMapOnId() =
    ...
    println(
      Monad[Id].flatMap(one)(_ * 100)
    ) // 100; same as map since `Id[B]` is just `B`
