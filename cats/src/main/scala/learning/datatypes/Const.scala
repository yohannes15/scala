...
/**
  ...
  * Part 3 (below) explains how that second name gets used when we recover
  * `get` from `modifyF`.
  *
  * Is `Const` only useful for lenses?
  * -----------------------------------
  * No. Phantom types and `Const` show up in many places (e.g. functor
  * composition, `traverse`/`Foldable` derivations, optics libraries, profunctors).
  * This file uses **lenses** as a concrete story: first `modifyF` unifies
  * `modify` / `Option` / `List`, then `Const` explains how to recover `get`
  * from `modifyF` alone.
  *
  * Why lenses?
  * -----------
  * Types that contain other types are everywhere. In OOP you might use getters
  * and setters; in FP a common abstraction is a **lens**: a small API for
  * focusing on a part `A` of a whole `S` (`get` / `set`, and derived updates).
  *
...
object LearningConst:
    ...
    object part1:
        /**
          ...
          *
          * Part 2 — `modifyOption` and `modifyList` differ only in the outer
          * type constructor (`Option` vs `List`). Both have `map`, i.e. they
          * are **Functors**. So one general API is `modifyF` for any `F` with
          * `Functor`.
          */
        trait Lens[S, A]:
            def get(s: S): A
    ...
    object part2:
        /**
          ...
          *
          * `modifyF` must stay abstract: if you defined it only from `set`,
          * you would circle back through methods that already depend on `modifyF`.
          */
        trait Lens[S, A]:
            def modifyF[F[_]: Functor](s: S)(f: A => F[A]): F[S]
    ...
    /**
      ...
      *
      * **Why name `B` in `Const[A, B]`?** The only runtime field is the `A`
      * in `getConst`, so a `Const[A, B]` value is "really" just that `A`. If
      * you swap `B` for another type, you do not add or remove data — you only
      * change a static type. That is the usual sense of a *phantom* type
      * parameter.
      *
      * **Why have two type parameters at all?** `modifyF` is polymorphic in
      * a type constructor `F[_]` (e.g. `List`, `Option`). Those functors use
      * that parameter to describe the inner type. `Const` is written with two
      * parameters so you can form `F[_] = Const[A, *]`: the stored value stays
      * an `A`, while the second parameter can still appear in types like
      ...
      *
      * You need a `Functor` instance for `Const[X, *]` (first parameter `X` fixed).
      */
    object part3:
    ...
    def part1Examples(): Unit =
        ...
        println(
          s"part1 modifyOption (bump if even): ${personAgePart1.modifyOption(p)(a => if a % 2 == 0 then Some(a + 1) else None)}"
        )
        println(
          s"part1 modifyOption (always None): ${personAgePart1.modifyOption(p)(_ => None)}"
        )
        println(s"part1 modifyList: ${personAgePart1.modifyList(p)(a => List(a, a + 1))}")
        println(
          s"part1 modifyF (List): ${personAgePart1.modifyF[List](p)(a => List(a, a + 1))}"
        )
    ...
    def part2Examples(): Unit =
        ...
        println(
          s"part2 modifyF Option: ${personAgePart2.modifyF[Option](p)(a => Some(a + 1))}"
        )
        println(
          s"part2 modifyF List: ${personAgePart2.modifyF[List](p)(a => List(a, a + 1))}"
        )

    def part3Examples(): Unit =
        val p = Person("Carol", 25)
        println(s"part3 get (derived via Const + modifyF): ${personAgePart3.get(p)}")
        ...
        println(
          s"part3 modifyF Option: ${personAgePart3.modifyF[Option](p)(a => if a < 30 then Some(a) else None)}"
        )
