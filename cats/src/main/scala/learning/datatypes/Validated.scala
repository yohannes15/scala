...
/** Validated --------------- Imagine you are filling out a web form to signup
  * for an account. You input your username and password and submit. Response
  * comes back saying your username can't have dashes in it, so you make some
  * changes and resubmit. Can't have special characters either. Change,
  * resubmit. Passwords need to have at least one capital letter. Change,
  ...
  *
  * Enter Validated!! -------------------- Note firsthand that `Validated` is
  * very similar to `Either` because it also has two possible values: errors on
  * the left side or successful computations on the right side.
  *
  * Validated vs Either -------------------- We've established that an
  * error-accumulating data type such as Validated can't have a valid Monad
  * instance. Sometimes the task at hand requires error-accumulation. However,
  * sometimes we want a monadic structure that we can use for sequential
  * validation (such as in a for-comprehension). This leaves us in a bit of a
  * conundrum.
  *
  * Cats has decided to solve this problem by using separate data structures for
  * error-accumulation (Validated) and short-circuiting monadic behavior
  * (Either).
  *
  ...
  *
  * Validated has helper sequential functions `andThen` and `withEither`
  * discussed below if needed.
  */
object LearningValidated:
  ...
  def forComprehensionisFailFast() =
    ...
    println(
      s"We should have gotten another DomainValidation object denoting the invalid age.\n"
    )
...
object ValidatedApproach:
  ...
  def validateFirstName(
      firstName: String
  ): Validated[DomainValidation, String] =
...
/** We have to look into another direction: a for-comprehension plays well in a
  * fail-fast scenario but the structure in our previous example was designed to
  * catch one error at a time, so, our next step is to tweak the implementation
  * a bit.
  *
  * Whats changed here? -------------------
  *   1. In this new implementation, we're using a `NonEmptyChain`, a data
  *      structure that guarantees that at least one element will be present. In
  *      case that multiple errors arise, you'll get a chain of
  *      DomainValidation.
  *   2. `ValidatedNec[DomainValidation, A]` ==
  *      `Validated[NonEmptyChain[DomainValidation], A]`. When you use
  *      ValidatedNec you're stating that your accumulative structure will be a
  *      NonEmptyChain. With Validated, you have the choice about which data
  *      structure you want for reporting the errors
  *   3. type alias `ValidationResult` that conveniently expresses the return
  *      type of our validation.
  *   4. `.validNec` and `.invalidNec` combinators lets you lift the success or
  *      failure in their respective container (either a Valid or
  *      Invalid[NonEmptyChain[A]]).
  *   5. The applicative syntax `(a, b, c, ...).mapN(...)` provides us a way to
  *      accumulatively apply the validation functions and yield a product with
  *      their successful result or the accumulated errors in the NonEmptyChain.
  *      Then, we transform that product with mapN into a valid instance of
  *      RegistrationData.
  */
sealed trait FormValidatorNec:
  ...
  /** Note that, at the end, we expect to lift the result of the validation
    * functions in a RegistrationData instance. If the process fails, we'll get
    * our NonEmptyChain detailing what went wrong.
    */
  def formValidatorNecExample() =
    ...
    println(
      s"Sweet success! Now you can take your validation process to the next level!"
    )
...
/** ** Combinators and SemiGroup with Validated
  ...
  * ValidatedNec[DomainValidation, A] is an alias for
  ...
  * expressed by the |+| operator is another NonEmptyChain with the three
  * elements.
  *
  * But, what about if we want another way of combining? We can provide our
  * custom Semigroup instance with the desired combining logic and pass it
  * implicitly/using to your scope.
  */
def necAndSemiGroupExample() =
  ...
  val res =
    NonEmptyChain.one[DomainValidation](UsernameHasSpecialCharacters) |+|
      NonEmptyChain[DomainValidation](
        FirstNameHasSpecialCharacters,
        LastNameHasSpecialCharacters
      )
...
/** From Validated to Either -------------------------- Cats offers us a nice
  * set of combinators for transforming your `Validated` based approach to an
  * `Either` one and vice-versa. We've seen `.toValidated`. There is also
  * .toEither
  */
def toEitherExample() =
  // Successful case
  val res = FormValidatorNec
    .validateForm(
      username = "Joe",
      password = "Passw0r$1234",
      firstName = "John",
    ...
    .toEither
  println(
    s"result of validateForm with success ValidatedNec converted toEither: $res\n"
  )
  ...
  val fail = FormValidatorNec
    ...
    .toEither
  println(
    s"result of validateForm with fail ValidatedNec converted toEither: $fail"
  )
  println(
    s"Sweet success! Now you can take your validation process to the next level!\n"
  )
...
object ParallelValidated:
  ...
  pieces of data.
   */
  def parallelValidateSimple[E, A, B, C](
      v1: Validated[E, A],
      v2: Validated[E, B]
...
/** Our `parallelValidate` function looks awfully like the `Apply#map2`
  * function:
  *
  * `def map2[F[_], A, B, C](fa: F[A], fb: F[B])(f: (A, B) => C): F[C]`
  *
  * Can we define an Apply instance for Validated? Better yet, can we define an
  * Applicative instance? Yes we can. Below is implementation of Applicative for
  * Validated.
  *
...
object ApplicativeValidated:
  ...
  given validatedApplicative[E: Semigroup]: Applicative[Validated[E, *]] =
    new Applicative[Validated[E, *]]:
      def ap[A, B](ff: Validated[E, A => B])(
          fa: Validated[E, A]
      ): Validated[E, B] =
        (fa, ff) match
...
/** VALIDATED HAS ONLY AN APPLICATIVE INSTANCE
  ...
  *
  * Note that all Monad instances are also Applicative instances, however, the
  * ap behavior defined in terms of `flatMap` does not behave the same as that
  * of our ap defined above. Observe:
  * validatedMonad.tuple2(Validated.invalidNec[String, Int]("oops"),
  * Validated.invalidNec[String, Double]("uh oh")) //
  * Validated[NonEmptyChain[String], (Int, Double)] = Invalid( e = Singleton(a =
  * "oops"))
  *
  ...
  * with that of ap, and this will violate one of the FlatMap laws,
...
object MonadValidated:
  ...
  def validatedMonadShortcircuits() =
    println(
      s"validatedMonad short circuits if you uncomment override def ap: ${validatedMonad.tuple2(
...
/** Sequential Validation ----------------------- If you do want error
  * accumulation but occasionally run into places where sequential validation is
  * needed, then Validated provides a couple methods that may be helpful.
  *
  * `andThen` ----------- similar to flatMap. In the case of success, it passes
  * the valid value into a function that returns a new `Validated` instance.
  * Apply a function (that returns a Validated) in the valid case. Otherwise
  * return the original Validated.
  *
  * This allows "chained" validation: the output of one validation can be fed
  * into another validation function. This function is similar to flatMap on
  * Either. It's not called flatMap, because by Cats convention, flatMap is a
  * monadic bind that is consistent with ap. This method is not consistent with
  * ap (or other Apply-based methods), because it has "fail-fast" behavior as
  * opposed to accumulating validation failures.
  *
  * `withEither` ------------- allows you to temporarily turn a Validated
  * instance into an Either instance and apply it to a function. Convert to an
  * Either, apply a function, convert back. This is handy when you want to use
  * the Monadic properties of the Either type.
  */
def validatedHelpersForSequentialValidation() =
  ...
  val fail2 = config
    .parse[Int]("house_number")
    .withEither(either => either.flatMap(i => positive("house_number", i)))
